...
Tests verify bitmap mode entry/exit, matrix copy, and bounds validation.
"""
...
class TestGraphicsEnter:
    ...
    def test_enter_bitmap_mode(self) -> None:
        """graphics_enter should switch to bitmap mode."""
        dll = _dll_path()
        ...
        addr = _load_symbol_address("graphics_enter")
        emu.set_x(0x00)
        emu.set_y(0x10)
...
class TestGraphicsExit:
    ...
    def test_exit_bitmap_mode(self) -> None:
        """graphics_exit should restore text mode."""
        dll = _dll_path()
        emu = C64Emu6502(lib_path=dll)
        bin_path = ROOT / "build" / "compiler.bin"
        if not bin_path.exists():
        ...
        load_addr = payload[0] | (payload[1] << 8)
        emu.write_mem_range(load_addr, payload[2:])
        addr = _load_symbol_address("graphics_exit")
        emu.set_a(0)
        emu.execute(addr, 10000)
        state = emu.get_state()
        assert state.a is not None
...
class TestGraphicsMatrixCopy:
    ...
    def test_matrix_copy(self) -> None:
        """graphics_matrix_copy should copy bitmap data with IRQ opportunities."""
        dll = _dll_path()
        emu = C64Emu6502(lib_path=dll)
        bin_path = ROOT / "build" / "compiler.bin"
        if not bin_path.exists():
        ...
        load_addr = payload[0] | (payload[1] << 8)
        emu.write_mem_range(load_addr, payload[2:])
        addr = _load_symbol_address("graphics_matrix_copy")
        emu.set_x(0x00)
        emu.set_y(0x10)
        emu.execute(addr, 10000)
        state = emu.get_state()
...
class TestGraphicsValidateBounds:
    ...
    def test_validate_bounds(self) -> None:
        """graphics_validate_bounds should check pixel/cell limits."""
        dll = _dll_path()
        emu = C64Emu6502(lib_path=dll)
        bin_path = ROOT / "build" / "compiler.bin"
        if not bin_path.exists():
