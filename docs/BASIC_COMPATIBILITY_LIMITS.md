...
| Tokenized line payload | A tokenized BASIC line is byte-counted and must fit the stock linked-line representation. Compiler 2 must reject or preserve-by-extension any line that cannot round-trip through stock BASIC V2 format. | Stock-only `SAVE` never emits malformed line records. Extended programs use the versioned C2P1 envelope (`DESIGN2.md` §5.2). | Stock format is enforced by the program encoder/decoder and canonical save tests. |
...
| Tokenization | Extended tokens must never remap or reinterpret a stock BASIC V2 token byte. Extended programs use the C2P1 envelope described in `REQUIREMENTS.md` / `DESIGN2.md` §5.2. |
